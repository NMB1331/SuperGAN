...
def compute_statistical_vector(X, y, feature_net, num_channels, num_classes, instances_per_class_train, instances_per_class_test):
    ...
    for label in range(num_classes):
        ...
        S_X_train_one_class = np.repeat(np.reshape(np.mean(feature_net.predict(selected_X), axis=0), (1, num_channels*9)), instances_per_class_train, axis=0)  
        S_X_test_one_class = np.repeat(np.reshape(np.mean(feature_net.predict(selected_X), axis=0), (1, num_channels*9)), instances_per_class_test, axis=0)
