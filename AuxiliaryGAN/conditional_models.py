...
from tensorflow.keras.layers import Dense, Flatten, Reshape, LSTM, Permute, Dropout, Lambda, LeakyReLU, Activation, Input
...
'''
Auxiliary GAN creation function
'''

NUM_SEQS = 1

#This will create the version of the discriminator that predicts the class label as
#well as the "realness" of the data
def create_AC_D(seq_length, num_channels, num_labels):
    D_input_shape = (seq_length, num_channels + num_labels)
    D_in = Input(shape=D_input_shape)
    D = Dropout(.5)(D_in)
    D = LSTM(100, activation="tanh")(D)
    D_out_real_fake = Dense(1, activation="sigmoid")(D)
    D_out_class_prediction = Dense(num_labels, activation="softmax")(D)
    D = Model(inputs=D_in, outputs=[D_out_real_fake, D_out_class_prediction], name="D")
    return D
...
def create_cond_G_multibranch(seg_length, num_channels, num_labels, latent_dim):
    ...
    generator = Reshape((seg_length,NUM_SEQS))(generator)
    generator = LSTM(NUM_SEQS, return_sequences=True, activation="tanh", name="g4")(generator)
