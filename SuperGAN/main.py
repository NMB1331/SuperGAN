...
fpath_data, fpath_classifier, class_label, model_save_directory = input_mod.parse_input_file(input_file)
X,y,y_onehot = input_mod.load_data(fpath_data, class_label)
write_train_results = True
# Gets SFD loss
try: 
    SFD_loss = float(sys.argv[2])
except: 
    SFD_loss = 0
...
accuracy_threshold = .9 #threshold to stop generator training

#WEIGHTS FOR DIFFERENT TERMS IN THE LOSS FUNCTION
D_loss_weight = 1
C_loss_weight = 1
SFD_loss_weight = SFD_loss

#OUTPUT FILE NAME
train_results_name = "SuperGAN" + "_SFD" + str(int(SFD_loss_weight)) + "_AccelData_Class" + str(class_label) + "_train_results.csv"
...
S_X_train = np.repeat(np.reshape(np.mean(feature_net.predict(X, batch_size), axis=0), (1, num_channels*9)), batch_size, axis=0)  
S_X_test = np.repeat(np.reshape(np.mean(feature_net.predict(X, batch_size), axis=0), (1, num_channels*9)), test_size, axis=0)  # made a vector with the averages to check against the generated data
...
while GC_acc<accuracy_threshold or epoch <= 100:
    ...
    mean_RTS_sim, mean_STS_sim, _ = train.compute_similarity_metrics(synthetic_data, X, test_size,\
    ...
    if write_train_results == True:
        save.write_results(train_results_name, epoch, class_label, D_acc, GD_acc, GC_acc, mean_RTS_sim, mean_STS_sim)
